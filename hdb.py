...
def add_recovery(torrent_id, torrent_folder):
    ...
    cmd = '/home/boxbox/bin/chtor %s.torrent --hashed=\'%s\''%(torrent_id, path)
    print(cmd)
    r = execute(cmd)
    write_log(r)
    write_log('Success')
...
def re_add_start(torrent_id):
    ...
    cmd = 'cp %s /home/ptrss/rtorrent/watch'%torrent_path 
...
def check_statue(torrent_id):
    ...
    cmd = '/home/boxbox/bin/rtxmlrpc d.connection_current %s'%torrent_id
